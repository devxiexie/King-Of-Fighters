...
export class Player extends AcGameObject {
	constructor(root, info) {
		...
		this.speedy = -1000; //jumping initial speed
		...
		this.status = 3; // 0: idle, 1: forward, 2: backwards, 3: jumping, 4: attacking, 5: getting hit, 6: dead
	...
	update_control() {
		let w, a, d, space, emote;
		if (this.id === 0) {
			w = this.pressed_keys.has("w");
			a = this.pressed_keys.has("a");
			d = this.pressed_keys.has("d");
			space = this.pressed_keys.has(" ");
			emote = this.pressed_keys.has("1");
		} else {
			w = this.pressed_keys.has("ArrowUp");
			a = this.pressed_keys.has("ArrowLeft");
			d = this.pressed_keys.has("ArrowRight");
			space = this.pressed_keys.has("Enter");
			emote = this.pressed_keys.has("0");
		}
	...
	render() {
		...
		if (obj && obj.loaded) {
			if (this.direction > 0) {
				...
				this.ctx.drawImage(
					image,
					this.root.game_map.$canvas.width() -
						this.x -
						this.width +
						obj.offset_x,
					this.y + obj.offset_y,
